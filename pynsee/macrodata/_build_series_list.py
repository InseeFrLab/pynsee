...
def _build_series_list(dt=["CNA-2014-ERE"]):
    ...
    series_list_new_title_missing = series_list_new[pd.isna(
        series_list_new['TITLE_FR'])]
    ...
    if len(list_series_title_missing) > 0:
        ...
        if len(titles.index) > 0:
            ...
            for i in range(len(series_list_new2.index)):
                ...
                if pd.isna(tt_fr):
                    series_list_new2.loc[i,
                                         "TITLE_FR"] = series_list_new2.loc[i, "TITLE_FR2"]

                if pd.isna(tt_en):
                    series_list_new2.loc[i,
                                         "TITLE_EN"] = series_list_new2.loc[i, "TITLE_EN2"]
