    ...
    if isinstance(df, pd.DataFrame):
        for title in title_col_name:
            if title in df.columns:
                try:
                    df_title_splitted = df[title].str.split(
                        pattern, n_split).tolist()
                except:
                    df_title_splitted = df[title].str.split(pattern).tolist()

                df_title_splitted = pd.DataFrame(
                    df_title_splitted, index=df.index)
                max_col = max(df_title_splitted.columns) + 1

                df_title_splitted.columns = [
                    title + str(num + 1) for num in range(max_col)]
