...
def get_activity_metadata(level, version='latest'):
    ...
    from pynsee.metadata._clean_naf_row import _clean_naf_row
    ...
    if level == 'A138':
        label_cleaned = _clean_naf_row(label)
        label_cleaned = _clean_naf_row(label_cleaned)
        label_cleaned['NAF'] = label_cleaned['NAF'].apply(drop_space)
